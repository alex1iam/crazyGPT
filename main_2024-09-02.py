...
devices = {
    "1": Device(
        main_topic='zigbee2mqtt/0xa40000d99e6fe4ee',
        name='розетка',
        location='зал',
        commands={
            'status': 'stat',
            ...
            '/': {  # Команды управления состоянием
                ...
                ('закрой', 'заблокируй', 'запри'): '{"child_lock": "LOCK"}',  # Добавлено новое действие
                ('открой', 'разблокируй', 'отопри'): '{"child_lock": "UNLOCK"}',  # Добавлено новое действие
            },
        },
        status='stat',
        sending='set'
      ...
      "2": Device(
        main_topic='zigbee2mqtt/0xa41111d99e6fe4ee',
        name='свет',
        location='кухня',
        commands={
            'status': 'stat',
...
def parse_command(command):
    ...
    for device_key, device_info in devices.items():
        ...
        if score_name > SIM_NAME and score_location > SIM_LOCATION:
            ...
            print(f"Найдено устройство: {device_name} в {location} (score_name: {score_name}, score_location: {s>
